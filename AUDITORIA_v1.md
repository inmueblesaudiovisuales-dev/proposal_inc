...
- **Corrección APLICADA:** por decisión del dueño se cambió el esquema de folios a letra correlativa por día (`PI-YYMM.DD-A`, `-B`, `-C`, ...). Cada contrato recibe su letra al crearse y nunca se modifica, así que ya no se re-folian contratos previos y el PDF/carpeta quedan siempre consistentes. Un folio antiguo sin sufijo cuenta como `A`.
...
- **P-07 (folio re-foliado en PDF):** RESUELTA y APLICADA. El dueño optó por cambiar el esquema a letra correlativa por día (`PI-YYMM.DD-A`, `-B`, ...). Elimina la re-foliación de raíz (A-09 corregido).
